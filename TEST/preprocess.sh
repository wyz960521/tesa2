...
bedtools intersect -a ${MACE_PREFIX}_out.tmp.bed -b ${MACE_PREFIX}_peaks_gaps.tmp.bed -wb | awk '{print $0 "\t" $3-$2}' | cut -f4,8,9 | awk '$1 != f1 {if(NR>1) print f2; f1=f2=s=""}{f1=$1; for(i=0;i<$3;i++) {f2=f2 s $2; s=","}}END { print f1 }' > coverage.tmp.txt

bedtools getfasta -fi ${REFERENCE_FILE} -bed ${MACE_PREFIX}_out.tmp.bed  |  awk '(NR)%2==0 {getline this<"coverage.tmp.txt";print this} 1' - | head -n -3 > ${OUTPUT_NAME}.tesa
